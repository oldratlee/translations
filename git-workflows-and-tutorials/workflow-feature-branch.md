...
`Pull Request`很酷的是可以在相关的提交旁边显示评注，所以可以很对某个变更集提问。
...
无论谁来做合并，首先要检出`master`分支并确认是它是最新的。然后执行`git pull origin marys-feature`合并`marys-feature`分支到和远程一致的本地`master`分支。
